...
def okbuck_lint(
    ...
    bash += "export ANDROID_LINT_JARS=\"$PREBUILT_AAR_LINT_JARS{}\"; PROJECT_ROOT=`echo '{}' | sed 's|buck-out.*||'`; ".format(':'.join([toLocation(custom_lint) for custom_lint in custom_lints]), toLocation(manifest))
    ...
    for src in srcs:
        bash += '--sources {}/{} '.format(native.package_name(), src)

    for res_dir in res_dirs:
        bash += '--resources {}/{} '.format(native.package_name(), res_dir)
...
def okbuck_manifest(
    ...
    if len(secondary_manifests) > 0:
        for secondary_manifest in secondary_manifests:
            cmds.append("--overlays {}".format("$(location " + secondary_manifest+")"))
